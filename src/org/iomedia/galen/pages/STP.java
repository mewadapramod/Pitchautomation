...
//	private By Logout =  By.xpath("//[@id='my-invoice']/li/a[contains(@href, 'user/logout')]");
	...
	public void buyGATickets(String paymentMode, String cardType, String cardNum, String ccExpiry, String CVV, TestDevice device){
	...
	public void enter_CCdetails(String ccNum, String expiry, String cvv){
...
//		click(mob_paymentPlan, "Payment plan" );
	...
	public void typeEmailAddress(String emailAddress, TestDevice device){
		if((device != null && (device.getName().trim().equalsIgnoreCase("mini-tablet") || device.getName().trim().equalsIgnoreCase("mobile"))) || driverType.trim().toUpperCase().contains("ANDROID") || driverType.trim().toUpperCase().contains("IOS")) {
	...
	public void typePassword(String password, TestDevice device){
		if((device != null && (device.getName().trim().equalsIgnoreCase("mini-tablet") || device.getName().trim().equalsIgnoreCase("mobile"))) || driverType.trim().toUpperCase().contains("ANDROID") || driverType.trim().toUpperCase().contains("IOS")) {
	...
	public void login(String emailaddress, String password, TestDevice device){
...
//		} while(isPasswordFieldVisible(device) && counter > 0);
	...
	private void type(By locator, By mobileLocator, String objName, String text, TestDevice device) {
		if((device != null && (device.getName().trim().equalsIgnoreCase("mini-tablet") || device.getName().trim().equalsIgnoreCase("mobile"))) || driverType.trim().toUpperCase().contains("ANDROID") || driverType.trim().toUpperCase().contains("IOS")) {
	...
	public void createaccount(TestDevice device, String suffix) {
	...
	public void typeCardNumber(String cardNum){
		type(cardNumber, "CC Number", cardNum);
	}
	
	public void typeCardExpiry(String expiry){
